  ...
  const handleFileUpload = async () => {
    if (selectedFile) {
      try {
        ...
        alert("File upload is currently simulated.");
        ...
        <div className="w-full max-w-3xl mx-auto">
            <div className="relative">
              <Input
                ...
                className="h-14 bg-[#121216] border-gray-700 text-white rounded-full pl-14 pr-24"
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
              />
              <Paperclip className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-gray-400" />
              <Button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 h-10 w-10 bg-blue-600 hover:bg-blue-700 rounded-full" size="icon" onClick={handleSendMessage}>
